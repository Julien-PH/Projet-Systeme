...
def consultation():
    ...
    FCS.send("consultation" + "/" + str(pidClient) + "/" + nomFichier + "/" + str(recupNumEnreg) + "/" + "-" , None, 3) #On lance le message en concÃ©quence, et avec toutes les informations utiles
...
def visualisation():
    FCS.send("visualisation" + "/" + str(pidClient) + "/" + nomFichier + "/" + "-" + "/" + "-", None, pidClient)    #On rÃ©alise une visualisation
    print("Veuillez patienter le temps que le serveur traite votre requete: "+str(pidClient))
    time.sleep(8)
...
def modification():
    ...
    FCS.send("consultation" + "/" + str(pidClient) + "/" + nomFichier + "/" + str(recupNumEnreg) + "/" + "-" , None, pidClient) #On rÃ©alise dÃ©jÃ  une consultation
    ...
    if choixModif == "O":
        ...
        FCS.send("modification" + "/" + str(pidClient) + "/" + nomFichier + "/" + str(recupNumEnreg) + "/" + str(NouvelEnreg) , None, 1)    #On rÃ©alise mtn une modification
        print("Veuillez patienter le temps que le serveur traite votre requete")
...
def suppression():
    ...
    FCS.send("consultation" + "/" + str(pidClient) + "/" + nomFichier + "/" + str(recupNumEnreg) + "/" + "-" , None, 2)  #On rÃ©alise dÃ©jÃ  une consultation
    print("Veuillez patienter le temps que le serveur traite votre requete")
    msgCons = FSC.receive(pidClient) #!
    print(format(msgCons))
